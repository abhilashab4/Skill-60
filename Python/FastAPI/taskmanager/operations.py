...
column_fiels = ["id", "title", "description", "status"]

def read_all_tasks() -> list[TaskWithID]:
    with open(DATABASE_FILENAME) as csvfile:
        reader = csv.DictReader(csvfile, fieldnames=column_fiels)
...
def write_task_into_csv(task: TaskWithID):
    with open(DATABASE_FILENAME, mode="a", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=column_fiels)
...
def modify_task(id : int, task: dict) -> TaskWithID | None:
    ...
    with open(DATABASE_FILENAME, mode="w", newline="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=column_fiels)
        writer.writeheader()
        for task in tasks:
